...
class SearchTree:
    ...
    def update_tree(self, model):
        ...
        if len(self.tree.graph["visited_node_ids"]) >= 1:
            self.prev_node_id, self.prev_node = (
                self.tree.graph["visited_node_ids"][-1],
                self.tree.graph["visited_nodes"][-1],
            )
        ...
        if len(self.tree.graph["visited_node_ids"]) > 2 or self.curr_node_id is None:
            if self.curr_node_id is not None:
                ...
                self.prev_node_id, self.prev_node = (
                    self.tree.graph["visited_node_ids"][-2],
                    self.tree.graph["visited_nodes"][-2],
                )
            else:
            ...
            if (
                len(list(self.tree.successors(self.prev_node_id))) == 0
                and self.prev_node_id != self.curr_node_id
            ):
    ...
    def _add_nodes(self, nodes, parent_node_id=None):
        ...
        for node_id, node in nodes.items():
            if node_id not in self.tree:
                # add node
                self.tree.add_node(
                    node_id, _id=node_id, lower_bound=node.getLowerbound()
                )
                ...
                if _parent_node is not None:
                    if parent_node_id is None:
                        ...
                        pass
                    self.tree.add_edge(parent_node_id, node_id)
                else:
    ...
    def _get_node_groups(self):
        ...
        for node in self.tree.nodes:
            if (
                node not in self.tree.graph["visited_node_ids"]
                or self.curr_node_id == node
            ):
    ...
    def render(
        ...
        if use_latex_font:
            sns.set(rc={"text.usetex": True}, font="times")
        ...
        nx.draw_networkx_labels(
            self.tree, pos, labels={node: node for node in self.tree.nodes}
        )
...
def create_milp_curriculum(
    num_levels, init_matrix_size, final_matrix_size, density=0.05
):
    """
    Args:
    ...
    nrows_delta, ncols_delta = int(nrows_distance / num_levels), int(
        ncols_distance / num_levels
    )

    curriculum = {0: [init_nrows, init_ncols]}
    for level in range(1, num_levels - 1):
        curriculum[level] = [
            curriculum[level - 1][0] + nrows_delta,
            curriculum[level - 1][1] + ncols_delta,
        ]
...
def check_if_network_params_equal(net_1, net_2):
    ...
    for (name_1, tensor_1), (name_2, tensor_2) in zip(
        net_1.named_parameters(), net_2.named_parameters()
    ):
        print(f"\n{name_1}:\n{tensor_1}:")
        print(f"{name_2}:\n{tensor_2}:")
...
class GraphDataset(torch_geometric.data.Dataset):
    ...
    def get(self, index):
        ...
        graph = BipartiteNodeData(
            sample_observation.row_features,
        ...
        graph.num_nodes = (
            sample_observation.row_features.shape[0]
            + sample_observation.variable_features.shape[0]
        )
...
class PlotAesthetics:
    ...
    def set_icml_paper_plot_aesthetics(
        self,
        context="paper",
        style="ticks",
        linewidth=0.75,
        ...
        # apply plot config
        sns.set(rc={"text.usetex": True, "figure.dpi": dpi, "savefig.dpi": dpi})
        sns.set(font="times")
        sns.set_theme(
            font_scale=font_scale, context=context, style=style, palette=palette
        )

    def get_standard_fig_size(
        self,
        col_width=3.25,
        col_spacing=0.25,
        n_cols=1,
        ...
        return (
            scaling_factor * width_scaling_factor * self.fig_width,
            scaling_factor * height_scaling_factor * self.fig_height,
        )

    def get_winner_bar_fig_size(self, col_width=3.25, col_spacing=0.25, n_cols=1):
        # save params
        self.col_width = col_width
        self.col_spacing = col_spacing
        self.n_cols = n_cols
...
def plot_val_line(
    ...
    class_colours = iter(
        sns.color_palette(palette="hls", n_colors=len(keys), desat=None)
    )
    for _class in sorted(plot_dict.keys()):
        if smooth_data_res is not None:
            ydata = smooth_line(
                plot_dict[_class]["y_values"], axis="y", res=smooth_data_res
            )
            xdata = smooth_line(
                plot_dict[_class]["x_values"], axis="x", res=smooth_data_res
            )
        else:
    ...
    if aspect != "auto":
        plt.gca().set_aspect(
            aspect=_get_matplotlib_aspect_ratio(fig, aspect_ratio=aspect)
        )
...
def sns_plot_val_line(
    ...
    **kwargs:
    ...
    class_colours = iter(
        sns.color_palette(
            palette=palette,
            n_colors=len(list(plot_dict.keys())) + len(list(horizontal_lines.keys())),
            desat=None,
        )
    )
    for _class in sorted(plot_dict.keys()):
