...
class RetroBranching:
    ...
    def extract(self, model, done):
        ...
        for node in nodes:
            if "score" not in self.normalised_lp_gain.tree.tree.nodes[node]:
                ...
                if node in self.normalised_lp_gain.tree.tree.graph["visited_node_ids"]:
                    ...
                    if self.debug_mode:
                        print(
                            f"Removing node {node} since was never visited by brancher."
                        )
                    self.normalised_lp_gain.tree.tree.graph["visited_node_ids"].remove(
                        node
                    )
        ...
        for node in self.normalised_lp_gain.tree.tree.graph["visited_node_ids"]:
            rewards.append(
                -np.log(self.normalised_lp_gain.tree.tree.nodes[node]["subtree"])
            )
        ...
        if self.debug_mode:
            ...
            print(
                f'Visited nodes: {self.normalised_lp_gain.tree.tree.graph["visited_node_ids"]}'
            )
            print(rewards)
