...
def seed_stochastic_modules_globally(
    default_seed=0, numpy_seed=None, random_seed=None, torch_seed=None, ecole_seed=None
):
...
def get_most_recent_checkpoint_foldername(path, idx=-1):
    ...
    idx_to_num = {
        idx: int(num[:-4])
        for idx, num in zip(
            range(len(foldernames)),
            [name[1] for name in foldernames if name[0] == "checkpoint"],
        )
    }
    sorted_indices = np.argsort(list(idx_to_num.values()))
    _idx = sorted_indices[idx]
    foldername = [
        name for name in os.listdir(path) if name.split("_")[0] == "checkpoint"
    ][_idx]
    return foldername
